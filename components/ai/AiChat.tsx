...
import {
    PDF_MARGIN, PDF_NAVY, createPdf, drawPdfFooter, drawPdfHeader, drawPdfTable,
    loadPdfStoreSettings, loadStoreLogo, pdfFileName, savePdf,
...
export const AiChat: React.FC<AiChatProps> = ({ userName, contextData, onClose, isFullScreen = false, initialGreetingContext }) => {
    ...
    const exportFullConversation = async () => {
        ...
        // Shares the app-wide masthead and footer; the body is free text rather
        // than a table, so it's laid out here.
        const settings = await loadPdfStoreSettings();
        const doc = createPdf();
        let y = drawPdfHeader(doc, {
            title: 'AI Chat Transcript',
            settings,
            logo: await loadStoreLogo(settings),
        });
        ...
        messages.forEach((msg) => {
            ...
            doc.text(msg.type === 'user' ? 'YOU' : 'AI ASSISTANT', PDF_MARGIN, y);
            y += 14;
        ...
        drawPdfFooter(doc, settings);
        savePdf(doc, pdfFileName('AI Chat Transcript', settings, currentConversationId || undefined));
    };
    ...
    const downloadPDF = async (data: ReportData) => {
        ...
        const startY = drawPdfHeader(doc, {
            ...
            settings,
            logo: await loadStoreLogo(settings),
            meta: ['Generated by the SalePilot AI Assistant'],
        });
        drawPdfTable(doc, { startY, head: [data.headers], body: data.rows });
        drawPdfFooter(doc, settings);
        savePdf(doc, pdfFileName(title, settings));
    };
                                            ...
                                            {message.reportData && message.isTyped && (
                                                <div className="mt-3 p-3 bg-slate-50 dark:bg-[#1e1f20] rounded-xl not-prose border border-slate-200 dark:border-[#333537] w-fit max-w-full">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button
                                                            onClick={() => { void downloadPDF(message.reportData!); }}
