        ...
        <div className={`p-4  bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)] z-10 ${className}`}>
            <div className={`flex ${compact ? 'flex-row' : 'flex-col sm:flex-row'} items-center justify-between gap-4`}>
                ...
                <div className="text-sm text-slate-500 font-medium whitespace-nowrap">
                    Showing <span className="text-slate-900 font-bold">{startItem}</span> - <span className="text-slate-900 font-bold">{endItem}</span> of <span className="text-slate-900 font-bold">{total}</span> {label}
                </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none pl-3 pr-8 py-2 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm transition-all hover:border-slate-300 cursor-pointer"
                            >
                                {[10, 20, 50, 100].map(sz => (
