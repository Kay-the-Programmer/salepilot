...
import { toneClass } from '../ui/StatusPill';
...
const PaymentStatusBadge: React.FC<{ status: Sale['paymentStatus'] | Sale['refundStatus'] }> = ({ status }) => {
    const statusConfig = {
        paid: { color: toneClass('success'), label: 'Paid' },
        unpaid: { color: toneClass('danger'), label: 'Unpaid' },
        partially_paid: { color: toneClass('primary'), label: 'Partial' },
        returned: { color: toneClass('neutral'), label: 'Returned' },
        partially_returned: { color: toneClass('amber'), label: 'Returned (Partial)' },
        fully_refunded: { color: toneClass('neutral'), label: 'Refunded' },
        partially_refunded: { color: toneClass('amber'), label: 'Refunded (Partial)' },
        none: { color: toneClass('neutral'), label: 'Draft' },
    };
    ...
    return (
        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest ${config.color}`}>
            {config.label}
        </span>
    );
};
