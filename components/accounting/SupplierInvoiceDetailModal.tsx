...
const SupplierInvoiceDetailModal: React.FC<SupplierInvoiceDetailModalProps> = ({ isOpen, onClose, invoice, onRecordPayment, storeSettings }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="liquid-glass-card rounded-[2rem] relative glass-effect !/95 dark:!bg-slate-900/95 w-full sm:max-w-2xl max-h-[96vh] sm:rounded-[2.5rem] rounded-t-[2.5rem] overflow-hidden flex flex-col animate-slide-up bg-gradient-to-b from-white to-slate-50/50 dark:from-slate-900 dark:to-slate-950/50">
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-8 sm:px-8 space-y-8 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="liquid-glass-card rounded-[2rem] p-5 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                                <div className="liquid-glass-card rounded-[2rem] p-2.5 dark:bg-slate-900 text-blue-600 dark:text-blue-400">
                            ...
                            <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                                <div className="liquid-glass-card rounded-[2rem] p-2.5 dark:bg-slate-900 text-indigo-600 dark:text-indigo-400">
                            ...
                            {invoice.payments.length > 0 ? (
                                <div className="space-y-3">
                                    {invoice.payments.map((p, idx) => (
                                        <div key={p.id || idx} className="liquid-glass-card rounded-[2rem] p-4 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-between group hover:border-blue-100 dark:hover:border-blue-900 transition-colors">
                                            <div className="flex items-center gap-4">
                                                <div className="p-2 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 rounded-xl group-hover:bg-blue-50 dark:group-hover:bg-blue-900 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors active:scale-95 transition-all duration-300">
                ...
                <div className="px-6 py-6 sm:px-8 border-t border-slate-100 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 sm:flex sm:flex-row-reverse gap-4">
                    {balanceDue > 0 && (
                        <button
                            ...
                            className="w-full sm:w-auto px-8 py-4 bg-blue-600 text-white rounded-[1.5rem] font-bold text-sm hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/10 active:scale-[0.98] flex items-center justify-center gap-2 active:scale-95 transition-all duration-300"
                        >
