...
export default function Sidebar({
    ...
    const getAppIconStyle = (page: string) => {
        ...
        if (page === 'reports' || page === 'superadmin' || page === 'customer/dashboard') return 'bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-blue-500/30';
    ...
    const getGroupedItems = () => {
        ...
        navItems.forEach(item => {
            if (['quick-view', 'reports', 'superadmin', 'customer/dashboard'].includes(item.page)) groups['Primary'].push(item);
            else if (['sales', 'sales-history', 'inventory', 'stock-takes', 'returns', 'purchase-orders', 'accounting'].includes(item.page)) groups['Operations'].push(item);
            else if (['customers', 'suppliers', 'users', 'directory', 'superadmin/stores'].includes(item.page)) groups['Relationships'].push(item);
            else if (['settings', 'superadmin/settings', 'audit-trail', 'subscription', 'superadmin/subscriptions', 'superadmin/notifications', 'whatsapp/conversations'].includes(item.page)) groups['System'].push(item);
            else groups['General'].push(item);
        });
