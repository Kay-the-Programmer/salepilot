...
import {
  createPdf, drawPdfFooterAsync, drawPdfHeader, drawPdfTable, loadPdfStoreSettings,
  loadStoreLogo, pdfFileName, savePdf,
...
const AssistantChat: React.FC<AssistantChatProps> = ({
  ...
  const downloadPDF = async (data: ReportData) => {
    ...
    // Same masthead, table style, footer and filename as every other export.
    ...
    const startY = drawPdfHeader(doc, {
      ...
      meta: ['Generated by the SalePilot Assistant'],
    });
    drawPdfTable(doc, { startY, head: [data.headers], body: data.rows });
    await drawPdfFooterAsync(doc, settings);
            ...
            <div className={`max-w-[88%] md:max-w-[75%] rounded-2xl px-4 py-3 text-[13.5px] shadow-sm ${m.type === 'user' ? 'chat-bubble-user rounded-br-md' : 'chat-bubble-ai rounded-bl-md'}`}>
              ...
              {m.reportData && (
                <div className="mt-3 flex flex-wrap items-center gap-2 pt-2 border-t m3-border-outline-variant">
                  ...
                  <button onClick={() => { void downloadPDF(m.reportData!); }} className="text-[11px] font-semibold px-2.5 py-1 rounded-lg m3-bg-primary m3-text-on-primary active:scale-95 transition">PDF</button>
