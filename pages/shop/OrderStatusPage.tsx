...
const OrderStatusPage: React.FC = () => {
    ...
    // Simple printable invoice (B2B buyers need a document for their books).
    // Rendered into a print window from the data already on screen — no
    // backend round-trip, works offline once the order is loaded.
    const printInvoice = (s: { orderId: string; timestamp?: string; total: number; subtotal?: number; tax?: number; deliveryFee?: number; customerName?: string; items: { name: string; quantity: number; price: number }[] }) => {
        const esc = (v: any) => String(v ?? '').replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c] as string));
        const storeName = esc(shopInfo.settings.name || shopInfo.name);
        const rows = (s.items || []).map(i =>
            `<tr><td>${esc(i.name)}</td><td class="n">${Number(i.quantity)}</td><td class="n">${esc(formatPrice(Number(i.price)))}</td><td class="n">${esc(formatPrice(Number(i.price) * Number(i.quantity)))}</td></tr>`
        ).join('');
        const line = (label: string, val: number | undefined) => val && val > 0
            ? `<tr class="t"><td colspan="3">${esc(label)}</td><td class="n">${esc(formatPrice(val))}</td></tr>` : '';
        const w = window.open('', '_blank', 'width=760,height=900');
        if (!w) return;
        w.document.write(`<!doctype html><html><head><title>Invoice ${esc(s.orderId)}</title><style>
            ...
            th,td{padding:8px 6px;border-bottom:1px solid #ddd;text-align:left} .n{text-align:right}
            tr.t td{border-bottom:none;padding-top:4px;font-weight:bold}
            .grand td{font-size:16px;border-top:2px solid #111}
            ...
            <tbody>${rows}</tbody>
            <tfoot>${line('Subtotal', Number(s.subtotal))}${line('Tax', Number(s.tax))}${line('Delivery fee', Number(s.deliveryFee))}
            <tr class="t grand"><td colspan="3">Total due</td><td class="n">${esc(formatPrice(Number(s.total)))}</td></tr></tfoot></table>
            <p class="muted" style="margin-top:32px">Generated by SalePilot · ${storeName}</p>
        </body></html>`);
        w.document.close();
        w.focus();
        w.print();
                ...
                <div className="px-6 py-5 border-b border-brand-border flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex gap-2 flex-wrap items-center">
                        ...
                        {summary.items.length > 0 && (
                            <button
                                onClick={() => printInvoice(summary as any)}
                                className="inline-flex items-center h-9 px-3.5 rounded-lg border border-sp-navy text-sp-navy text-xs font-bold hover:bg-sp-navy/5 transition-colors active:scale-95"
                            >
                                Print invoice
                            </button>
                        )}
