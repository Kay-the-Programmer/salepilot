 ...
 */
export async function registerStoreAndRefreshUser(name: string, businessTypes: string[] = [], phone?: string, address?: string, isWholesaleSupplier?: boolean): Promise<{ store: RegisterStoreResponse['store']; user: User }> {
  if (!name || name.trim().length < 2) {
    throw new Error('Please provide a valid store name (min 2 characters).');
  }
  const resp = await api.post<RegisterStoreResponse>('/stores/register', { name: name.trim(), businessTypes, phone, address, isWholesaleSupplier: isWholesaleSupplier === true }, { skipQueue: true });
